...
import { selectControls, selectSelectedPlayer, selectStage } from "./selectors";
...
function Controls(props) {
  ...
  const doButtonAction = async () => {
    ...
    const r = await fetch(
      make_api_url(props.game_tag, controlsState.button_submit_func, {
        ...
        {controlsState.button_visible ? (
          <>
            <Button
              ...
              variant={controlsState.button_enabled ? "primary" : "success"}
              size="lg"
              block
              disabled={!controlsState.button_enabled || isSending}
