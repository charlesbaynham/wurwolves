...
class WurwolvesGame:
    ...
    def join(self, user_id: UUID):
        ...
        with session_scope() as session:
            ...
            game = session.query(Game).filter(Game.id == self.game_id).first()

            if not game:
                game = self.create_game()

            # Add this user to the game as a spectator if they're not already in it
            player = session.query(Player).filter(Player.game == game and Player.user == user).first()
